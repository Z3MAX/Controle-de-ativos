...
const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        const savedTeam = localStorage.getItem('asset_manager_team');
        if (savedUser) {
  ...
  const selectTeam = async (team) => {
    try {
      ...
      localStorage.setItem('asset_manager_team', JSON.stringify(team));
      return { success: true };
    } catch (error) {
  ...
  const signOut = async () => {
    try {
      ...
      localStorage.removeItem('asset_manager_team');
      return { success: true };
    } catch (error) {
...
const TeamSelectionModal = ({ isOpen, onClose, onTeamSelect }) => {
  const { availableTeams } = useAuth();
        ...
        <div className="relative z-10">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 mt-3 font-medium">
              Escolha a equipe para gerenciar os ativos
            </p>
          </div>
...
const AssetControlSystem = () => {
  const { user, profile, selectedTeam, availableTeams, loading: authLoading, dbReady, signOut, selectTeam } = useAuth();
  ...
  useEffect(() => {
    if (user && dbReady && selectedTeam) {
      loadData();
    }
  }, [user, dbReady, selectedTeam]);
  
  useEffect(() => {
    if (user && dbReady && availableTeams.length > 0 && !selectedTeam) {
      setShowTeamSelection(true);
    }
  }, [user, dbReady, availableTeams, selectedTeam]);

  const loadData = async () => {
    if (!user || !user.id || !selectedTeam) return;
    
    setIsLoading(true);
    try {
      const [floorsResult, assetsResult] = await Promise.all([
        databaseService.floors.getAll(selectedTeam.id),
        databaseService.assets.getAll(selectedTeam.id)
      ]);
  ...
  const handleSaveRoom = async () => {
    ...
    if (!user || !user.id || !selectedTeam) {
      alert('Erro: usuário não autenticado ou equipe não selecionada');
      return;
    }
    ...
    try {
      ...
      let result;
      if (editingRoom) {
        result = await databaseService.rooms.update(editingRoom.id, roomData, selectedTeam.id);
      } else {
        result = await databaseService.rooms.create(roomData, selectedTeam.id);
      }
  ...
  const handleDeleteRoom = async (roomId) => {
    ...
    try {
      ...
      const result = await databaseService.rooms.delete(roomId, selectedTeam.id);
  ...
  const handleSaveAsset = async () => {
    ...
    try {
      ...
      if (!editingAsset) {
        const codeCheck = await databaseService.assets.checkCodeExists(assetForm.code, null, selectedTeam.id);
        if (codeCheck.success && codeCheck.exists) {
          alert('Já existe um ativo com este código nesta equipe.');
          return;
        }
      }
      ...
      let result;
      if (editingAsset) {
        result = await databaseService.assets.update(editingAsset.id, assetData, selectedTeam.id, user.id);
      } else {
        result = await databaseService.assets.create(assetData, selectedTeam.id, user.id);
      }
  ...
  const handleDeleteAsset = async (assetId) => {
    ...
    try {
      ...
      const result = await databaseService.assets.delete(assetId, selectedTeam.id);
  ...
  const handleTeamSelect = async (team) => {
    ...
    if (result.success) {
      // Recarregar dados após trocar de equipe
      setFloors([]);
      setAssets([]);
      await loadData();
    }
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex items-center space-x-4 text-gray-700 group hover:text-purple-600 transition-colors">
                  ...
                  <span className="font-medium">🏢 Gestão inteligente por equipes</span>
                </div>
                <div className="flex items-center space-x-4 text-gray-700 group hover:text-purple-600 transition-colors">
              ...
              <div className="mt-6 text-center">
                <p className="text-sm text-gray-500 font-medium">
                  <span className="text-green-600">✅ Gestão por equipes</span> • 
  ...
  // Se usuário está logado mas não selecionou equipe, mostrar seleção de equipe
  if (user && !selectedTeam && availableTeams.length > 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-cyan-50 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-4">🏢 Selecione sua Equipe</h2>
          <p className="text-gray-600 mb-8">Escolha a equipe para começar a gerenciar ativos</p>
          <button
            onClick={() => setShowTeamSelection(true)}
            className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-8 py-4 rounded-2xl font-bold transition-all transform hover:scale-105 shadow-lg"
          >
            🚀 Escolher Equipe
          </button>
        </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3">
              <div className="hidden lg:flex items-center space-x-4 text-sm">
                {selectedTeam && (
                  <div 
                    className="flex items-center space-x-2 px-4 py-2 rounded-xl border-2 cursor-pointer hover:shadow-lg transition-all"
                    style={{ 
                      backgroundColor: selectedTeam.color + '20',
                      borderColor: selectedTeam.color + '40'
                    }}
                    onClick={() => setShowTeamSelection(true)}
                    title="Clique para trocar de equipe"
                  >
                    <div 
